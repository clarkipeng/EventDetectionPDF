...
The two benchmarks differ by design.
...
We then repeat the strongest BDL and segmentation objectives with one-dimensional U-Nets and an attention-gated U-Net variant \citep{ronneberger2015,oktay2018attention}.
...
These segmentation-oriented backbones test whether the gap is explained by the recurrent encoder rather than by the training target.
Offline Transformer diagnostics are reported separately because they add a self-attention detector family with different sequence-length, memory, and optimizer tradeoffs \citep{vaswani2017attention}.
Those objective comparisons still use matched scoring and decoder calibration, but they are interpreted as backbone-specific scope checks rather than as simple skip-connection variants.
...
This protocol treats optimization and calibration as controlled experimental conditions rather than objective-specific method components.
