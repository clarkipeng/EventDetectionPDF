...
if [[ ! -x "$EVENTPDF_PY" ]]; then
  ...
  echo "Create the uv environment first, e.g. uv venv --python /usr/bin/python3.9 .venv && uv pip install --python .venv/bin/python -r requirements.txt" >&2
  exit 1
fi
...
run_phase "sleep_core_architectures" \
  "$EVENTPDF_PY" paper/run_experiments.py \
  "${BASE_ARGS[@]}" \
  --models gru unet unet_t prectime \
  --objectives density_hard hard seg \
  --seeds 0 1 2 \
  --run_tag objective_e20_bs32_eval5 \
  --wandb_group P3-main-table \
  --wandb_tags paper-density-likelihood-rewrite,sleep,P3
...
run_phase "sleep_online_models" \
  ...
  --wandb_group P7-online-ablation \
  --wandb_tags paper-density-likelihood-rewrite,sleep,P7

if "$EVENTPDF_PY" paper/check_data.py --datasets seizure --seizure-dir data/seizure >/dev/null 2>&1; then
  run_phase "seizure_replication" \
    "$EVENTPDF_PY" paper/run_experiments.py \
    --python "$EVENTPDF_PY" \
    --datasets seizure \
    ...
    --wandb_project event-detection-pdf \
    --wandb_mode "$WANDB_MODE" \
    --wandb_group P8-seizure-replication \
    --wandb_tags paper-density-likelihood-rewrite,seizure,P8 \
    --wandb_log_artifacts True \
    --skip-existing \
    --execute \
    --run_tag seizure_main_e20_bs32_eval5
else
  echo "Skipping seizure_replication: data/seizure is missing required files." | tee -a "$LOG_FILE"
  json_log "phase_skipped" "seizure_replication" "" "missing data/seizure"
fi

json_log "run_end" "all" "" "RUN_ID=$RUN_ID"
echo "finished_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$RUN_DIR/manifest.txt" | tee -a "$LOG_FILE"
echo "Status JSONL: $STATUS_JSONL"
