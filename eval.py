...
def evaluate(
    ...
    for step,(X, y, mask, id) in enumerate(tqdm(loader, desc="Eval", unit="batch")):
        ...
        (onsets, wakeups), (onsets_score, wakeups_score) = get_candidates(prediction, objective, max_distance, day_length)

        data_len = len(dataset.data[id])
        onset = dataset.data[id][['step']].iloc[np.clip(onsets*downsample+downsample//2,0,data_len-1)].astype(np.int32)
        onset['event'] = 'onset'
        onset['series_id'] = id
        onset['score'] = onsets_score
        wakeup = dataset.data[id][['step']].iloc[np.clip(wakeups*downsample+downsample//2,0,data_len-1)].astype(np.int32)
        wakeup['event'] = 'wakeup'
        wakeup['series_id'] = id
        ...
        del onsets,wakeups,onset,wakeup;gc.collect()
...
def get_optimal_cutoff(objective : str,
    ...
    def get_score(cutoff, smooth_param, tolerances):
        ...
        for id in dataset.ids:
            ...
            (onsets, wakeups), (onsets_score, wakeups_score) = get_candidates(pred, objective, max_distance, day_length, threshold = cutoff, smooth = smooth_param)
