...
These results do not displace segmentation for state estimation; they show
that event-localization benchmarks benefit from training on event locations
directly.
\end{abstract}
...
For interval events, the useful output may be the beginning or end of an
episode; for point events, it may be the time of an alarm, changepoint, or
clinical action \citep{shoeb2010,goldberger2000,alexander2017,esper2023}.
...
These precedents make localized boundary targets prior art.
...
For interval states, pairing, alternation, or duration constraints belong in
decoding; the likelihood estimates event occurrences, not complete interval
structure.
...
This paper introduced Boundary Density Likelihood, a count-preserving
likelihood objective for health time-series event detectors.
...
The experiments show that this objective can improve event AP when the
surrounding detector is held fixed.
On sleep-event detection, the gains are largest at strict endpoint tolerances,
persist across recurrent and convolutional backbones, and are not recovered by
class weighting, focal loss, or smoothing alone.
...
Future work should study stronger structured decoders for interval constraints
and repeated occurrences, calibrated online variants, and broader health
