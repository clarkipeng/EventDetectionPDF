...
run_score_if_ready() {
  ...
  local scores_path="experiments/seizure/${model}/${objective}/seed_0_${RUN_TAG}/results/scores.csv"
  if [[ -s "$scores_path" ]]; then
    echo "Skipping ${model}/${objective}; ${scores_path} already exists."
    return 0
  fi
  if [[ -d "$pred_dir" ]] && compgen -G "$pred_dir/*.npy" > /dev/null; then
    "${PY[@]}" "${COMMON[@]}" --model "$model" --objective "$objective"
  else
    echo "Skipping ${model}/${objective}: no predictions in ${pred_dir}"
  fi
}

for model in gru unet; do
  for objective in density_hard density_gau seg; do
    run_score_if_ready "$model" "$objective"
  done
done
