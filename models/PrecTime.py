...
class PrecTime(nn.Module):
    def __init__(
        ...
        self.feature_extraction1 = nn.Sequential(*feature_extraction1_layer)
        ...
        self.feature_extraction2 = nn.Sequential(*feature_extraction2_layer)

        self.fc1 = nn.Linear(
            self.hidden_channels * 2 * (self.sequence_length // self.chunks // 2), 64
        )
        ...
        self.inter_upsample = nn.Upsample(
            scale_factor=self.sequence_length // self.chunks, mode="nearest"
        )
        self.inter_fc = nn.Linear(
            in_features=self.context_detection2.hidden_size * 2, out_features=3
        )

        self.inter_upsample_di = nn.Upsample(
            scale_factor=self.sequence_length // self.chunks // 2, mode="nearest"
        )

        self.prediction_refinement = nn.Sequential(
            conv1d_block(
                in_channels=self.hidden_channels * 2
                + self.context_detection2.hidden_size * 2,
                out_channels=self.hidden_channels,
                kernel_size=self.kernel_size,
                padding=2,
                stride=self.stride,
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        x = x.reshape(-1, self.input_channels, x.shape[-1] // self.chunks)
        ...
        ui = (
            features_combined.transpose(0, 1)
            .reshape(features_combined.shape[1], origin_x.shape[0], -1)
            .transpose(0, 1)
        )
