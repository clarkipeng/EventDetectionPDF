...
def process_sleep_dataset(data_dir : str, 
    ...
    if not os.path.isfile(processed_filepath):
        ...
        for series_id, viz_series in tqdm(series[['series_id', 'step']+feats].groupby('series_id', sort=False)):
            data[series_id] = viz_series
            targets[series_id] = ([],[])
            
        for (series_id, event) in tqdm(series.loc[series.event.isin(['onset','offset']),['series_id','event','step',]].groupby('series_id', sort=False)):
            onset,offset = [],[]

            for i in range(len(event)-1):
                if event.iloc[i].event=='onset' and event.iloc[i+1].event=='offset':
...
class SleepDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = get_targets(self.dataclass, X.shape[0], self.targets[series_id], self.target_type, normalize = self.normalize)
        ...
        if self.training:
            ...
            for i in range(len(Xs)):
                Xs[i] = downsample_feats(Xs[i], self.downsample, self.cat_feats, self.agg_feats)
