...
WAIT_PIDS="${WAIT_PIDS:-}"
RUN_DIR="$REPO_ROOT/paper/generated_runs/finalize_after_parallel_lanes_$(date -u +%Y%m%dT%H%M%SZ)"
LOG_FILE="$RUN_DIR/finalize.log"
STATUS_FILE="$RUN_DIR/status.txt"
mkdir -p "$RUN_DIR"

echo "waiting for pids: $WAIT_PIDS" > "$STATUS_FILE"
{
...
for pid in $WAIT_PIDS; do
  while kill -0 "$pid" >/dev/null 2>&1; do
    echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) waiting for pid $pid" | tee -a "$LOG_FILE"
    sleep 300
  done
done
