...
def train(
    ...
    for fold in range(folds):
        ...
        scheduler = CosineLRScheduler(optimizer, t_initial = steps,warmup_t = warmup_steps, warmup_lr_init = 1e-6, lr_min=1e-6, cycle_mul = 1,)
        ...
        for epoch in range(1, epochs + 1):
            model, train_loss = train_epoch(train_loader,
                                            loss_fn,
                                            model, 
                                            epoch,
                                            scheduler,
        ...
        del train_dataset, train_loader, val_dataset, model, optimizer, scheduler, history; gc.collect()
