...
\usepackage[dblblindworkshop,nolinenumbers]{neurips_2026}
...
Boundary Density Likelihood changes the unit learned by a health time-series
detector.
When a long recording is ultimately reduced to a ranked list of onsets,
offsets, or alarms, samplewise segmentation asks the model to solve a proxy
problem and then repair the mismatch in post-processing.
BDL instead gives each annotated boundary a conserved count, fits the
conditional expected count on the model timeline, and decodes peaks from the
same event-score sequences that event AP later matches.

This matters because sparse health events are often the points at which timing
errors, duplicate detections, and false positives carry the largest operational
cost: missed episode starts, extra review burden, and weaker triage.
The current evidence shows that a likelihood-level reframe can improve event
AP across sleep and seizure settings without changing the backbone.
The state-reconstruction diagnostic keeps the claim honest: segmentation
remains the direct objective when the deployed quantity is samplewise
occupancy.
BDL is therefore best viewed as a simple, interpretable bridge between
time-discretized sequence models and event-level decision interfaces.
